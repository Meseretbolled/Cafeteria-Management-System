...
struct RemoteSoldItem {
    ...
    int quantity;
    double totalIncome;
     double totalCost; // New field to store the cost of ingredients
};
...
public:
    ...
    double loadPreviousIncome() {
        ...
        if(file.is_open()==false) {
            cerr << "Error: Could not open sales file"<<endl;
            return 0;
        }
        file >> income;
        return income;
...
private:
    double totalProfit;
     double loadTotalProfitFromFile() {
        ifstream file("total_profit.txt");
        double profit = 0.0;
        if (file.is_open()) {
            file >> profit;
            file.close();
    ...
    void saveTotalProfitToFile() {
        ofstream file("total_profit.txt");
        if (file.is_open()) {
            file << totalProfit;
            file.close();
        }
   ...
   RemoteOrderManager() {
        totalProfit = loadTotalProfitFromFile(); // Load existing total profit from file
    }
    ...
    void remoteOrder(MenuManager& menuManager, PrepaidCardManager& prepaidCardManager) {
    BusinessManager businessManager;
    string itemName;
    string address;
    int quantity;
    ...
    if (card != nullptr) {
        ...
        bool itemFound = false;
            for (MenuItem& menuItem : menuManager.menu) {
                if (menuItem.name == itemName) {
                    ...
                    double totalProfitFromOrder = 0.0; 
                    totalProfitFromOrder = totalPrice - totalCost;
                    totalProfit += totalProfitFromOrder;
                    
                    saveTotalProfitToFile();
    ...
    do {
        ...
        switch (menuChoice) {
            ...
            case 2:
                ...
                menuManager.saveMenuToFile("menuu.txt");
                break;
            case 3:
    ...
    do {
        ...
        switch (businessChoice) {
            ...
            case 2:
                ...
                break;
           case 3:
                totalProfit = remoteOrderManager.getTotalProfit();
...
void runMainMenu(MenuManager& menuManager, IngredientManager& ingredientManager, PrepaidCardManager& prepaidCardManager, BusinessManager& businessManager, RemoteOrderManager& remoteOrderManager, double& totalSales) {
    ...
    do {
        ...
        switch (mainChoice) {
            ...
            case 4:
                system("cls");
                remoteOrderManager.remoteOrder(menuManager,prepaidCardManager);
                menuManager.saveMenuToFile("menuu.txt");
...
int main() {
    ...
    menuManager.readMenuFromFile("menuu.txt");
